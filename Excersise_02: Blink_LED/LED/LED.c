...
void LED_init(void)
{
    ...
    IP_PORTD->PCR[LED_RED_PIN]   = PORT_PCR_MUX(1);
    IP_PORTD->PCR[LED_GREEN_PIN] = PORT_PCR_MUX(1);
    IP_PORTD->PCR[LED_BLUE_PIN]  = PORT_PCR_MUX(1);

    /* Set LED pins as output */
    IP_PTD->PDDR |= (1 << LED_RED_PIN) | (1 << LED_GREEN_PIN) | (1 << LED_BLUE_PIN);
...
{
    IP_PTD->PCOR = (1 << led); // Active low
}

void LED_off(uint8_t led)
{
    IP_PTD->PSOR = (1 << led); // Active high
}
